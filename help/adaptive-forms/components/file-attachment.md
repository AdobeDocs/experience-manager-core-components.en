...
- **Data type of submitted value**: Select the option to determine how the attached file is submitted. To send the file as binary data, choose the `File` option. To send the file as a Base64-encoded string, choose the `String` option. By default, the `File` option is selected. 
